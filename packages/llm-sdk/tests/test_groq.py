...
from llm_sdk.providers.groq import GROQ_BASE_URL, GroqProvider
...
async def test_complete_returns_text_and_token_meta():
    ...
    text, meta = await provider.complete([ChatMessage(role="user", content="hi")], "llama-3.3-70b-versatile")
...
async def test_stream_yields_text_then_final_with_usage():
    ...
    out = [pair async for pair in provider.stream([ChatMessage(role="user", content="hi")], "llama-3.1-8b-instant")]
...
async def test_stream_emits_sentinel_when_no_usage_chunk():
    """Still emit exactly one final sentinel even if no usage chunk arrives."""
    provider = GroqProvider(api_key="x")
